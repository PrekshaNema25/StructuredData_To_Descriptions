...
import numpy as np
from basic_files.vad_hier_dist_eval import *
from basic_files.rnn_cell_eval import *
...
class BasicAttention:
    ...
    def inference(self, encoder_inputs1, decoder_inputs1, query_inputs,field_inputs,sequence_length_inputs,  embedding_size, feed_previous,
        ...
        outputs, state, attention_weights_tokens, attention_weights_fields, forget_gate  = vad_seq2seq(encoder_inputs = ei,
                                                decoder_inputs = di,
                                                query_inputs = qi,
						field_inputs = fi, 
	...
	print ("Print length", len(outputs))
        ...
        #arand  = np.random.rand(46,3,10)
        #arand = arand.astype(np.float32)
        #self.final_outputs = tf.Variable(arand)

        #self.final_outputs = tf.unpack(self.final_outputs)

        #awts = np.random.randn(46, 3, 75)
        #awts = awts.astype(np.float32)
        #attention_weights_tokens = tf.Variable(awts)
        #attention_weights_tokens = tf.unpack(attention_weights_tokens)
        return self.final_outputs, attention_weights_tokens, attention_weights_fields, forget_gate
