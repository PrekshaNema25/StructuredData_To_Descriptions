...
from basic_files.vad_log_pemb import *
...
class BasicAttention:
    ...
    def inference(self, encoder_inputs1, decoder_inputs1, query_inputs, embedding_size, feed_previous,
        ...
        outputs, state, z  = vad_seq2seq(encoder_inputs = ei,
                                         decoder_inputs = di,
                                         query_inputs = qi,
	...
	self.final_z = z
        ...
        return self.final_outputs, self.final_z


    def loss_op(self, outputs, labels, weights, z, z_labels, len_vocab, mu):
	...
	_z_labels = tf.unpack(z_labels)
        weights = tf.to_float(weights)
        _weights = tf.unpack(weights)
        #print(_weights[0].get_shape())
        loss_per_batch = tf.nn.seq2seq.sequence_loss(outputs, _labels, _weights)
	loss_per_sw_batch = tf.nn.seq2seq.sequence_loss(z, _z_labels, _weights)

        self.calculated_loss =  loss_per_batch + loss_per_sw_batch
        return self.calculated_loss, loss_per_batch, loss_per_sw_batch
