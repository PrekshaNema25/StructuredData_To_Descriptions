...
class BasicAttention:
    ...
    def loss_op(self, outputs, labels, weights, len_vocab, attention_weights_fields, tau, l_lambda ):
	...
	loss_attn = 0.0
	for a  in attention_weights_fields:
            temp = 2 - a
            temp = a * temp 
            loss_attn += tf.reduce_sum(temp)



        loss_stay = 0.0
	for j in range(1,len(attention_weights_fields)):
            temp = attention_weights_fields[j] - attention_weights_fields[j-1]
	    temp = temp * temp 
	    loss_stay += tf.reduce_sum(temp)
	...
	batch_size = attention_weights_fields[0].get_shape()[0].value
        loss_per_batch = sequence_loss(outputs, _labels, _weights) + (loss_attn + loss_stay)/batch_size
