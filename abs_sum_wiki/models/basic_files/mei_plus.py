...
def vad_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    def attention_f(name):
      ...
      for a in xrange(num_heads):
        with variable_scope.variable_scope("Attention_f" +  name):
          # Attention mask is a softmax of v^T * tanh(...).
          s = math_ops.reduce_sum(
              v_field_state[a] * math_ops.tanh(hidden_features_field_states[a]), [2, 3])
          a = math_ops.sigmoid(s)
          # Now calculate the attention-weighted vector d.
      return  a
    ...
    attn_vec_fields = attention_f("beta")
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      x = linear([inp] + new_attns_state, input_size, True)
      ...
      # num_fields_g: Number of fields per box
      #attn_vec_tokens = array_ops.reshape(attn_vec_tokens, [batch_size, num_fields_g, -1])
      #new_attn_weights = math_ops.mul(attn_vec_tokens, attn_vec_fields)

      new_attn_weights = attn_vec_tokens * attn_vec_fields
      ...
      with variable_scope.variable_scope("AttnOutputProjection"):
        ...
        output = linear([cell_output] + new_attns_state + [inp], output_size, True)
  ...
  with variable_scope.variable_scope(
    ...
    with variable_scope.variable_scope("Encoder_Cell"):
      for i in range(num_fields_g):
          ...
          cell_encoder_bw, embedded_temp_inputs, sequence_length=sequence_field_length[i], dtype=dtype)
    ...
    top_states_field_encoder_preselect = []
    for i in range(len(field_outputs)):
    	top_states_field_encoder_preselect.append(array_ops.concat(1, [field_outputs[i], field_inputs[i]]))

    field_size = 2*cell_encoder_fw.output_size + field_inputs[0].get_shape()[1].value

    top_states_field_encoder_preselect = [array_ops.reshape(e, [-1, 1, field_size])
                                for e in top_states_field_encoder_preselect]

    attention_states_fields_preselect = array_ops.concat(1, top_states_field_encoder_preselect)
    ...
    if isinstance(feed_previous, bool):
      return vad_decoder_wrapper(
          ...
          attention_states=attention_states_fields_preselect,
          attention_states_fields = attention_states_fields_preselect,
