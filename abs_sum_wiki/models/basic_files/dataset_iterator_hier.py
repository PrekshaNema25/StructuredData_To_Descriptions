...
class Datatype:
    ...
    def __init__(self, name,title,label, content, query,field, sequence_length_field, exm,max_length_content, max_length_title, max_length_query):
...
class PadDataset:
    ...
    def next_batch(self, dt, batch_size, c=True):
        ...
        field, _ = self.make_batch(dt.field, batch_size, count, 15) 
        sequence_length_field, _ = self.make_batch(dt.sequence_length_field, batch_size, count, 15) 
	...
	max_l = 0
	   ...
	   sequence_length_list.append(temp)
        ...
        return Datatype(name, title_encoded, label_encoded, content_encoded, query_encoded, field_encoded, sequence_length_list, len(title_encoded), max_content, max_title, max_query)
