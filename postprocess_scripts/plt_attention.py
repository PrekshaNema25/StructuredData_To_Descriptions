...
def plot_attention(data, itr , X_label=None, Y_label=None, directory_name):
    '''
    ...
    if X_label != None and Y_label != None:
        ...
        ax.set_xticklabels(X_label, minor=False, rotation=30, fontsize=30)  # labels should be 'unicode'
    ...
    file_name = directory_name + '/attention_heatmap_' + str(itr) + ".jpg"
...
if __name__ == "__main__":
    ...
    x_label_file = open(sys.argv[3])
    y_label_file = open(sys.argv[2])
    max_count = int(sys.argv[4])
    directory_name = sys.argv[5]
    #forget_gate = pickle.load(open("test_fg"))
	...
	data_per_batch = awf[i]
	#fg_per_batch = forget_gate[i]
	data_per_batch = np.transpose(data_per_batch, axes=[1,0,2])
	#fg_per_batch = np.transpose(fg_per_batch)
	print (np.shape(data_per_batch))
		...
		plot_attention(data_current, count, x_label[count], y_label[count], directory_name)
		count = count + 1 
